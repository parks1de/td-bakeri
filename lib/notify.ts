...
export async function sendOwnerNotification(order: OrderData): Promise<void> {
    ...
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #1a1a1a; color: #f0e6d0; border-radius: 12px; overflow: hidden;">
      ...
      <div style="padding: 32px;">
        <table style="width: 100%; border-collapse: collapse;">
          <tr><td style="padding: 8px 0; color: #b8960c; font-weight: bold; width: 40%;">Navn:</td><td style="padding: 8px 0;">${order.name}</td></tr>
          ...
          <tr><td style="padding: 8px 0; color: #b8960c; font-weight: bold;">Storrelse:</td><td style="padding: 8px 0;">${order.size ?? "Ikke spesifisert"}</td></tr>
          <tr><td style="padding: 8px 0; color: #b8960c; font-weight: bold;">Beskrivelse:</td><td style="padding: 8px 0;">${order.description}</td></tr>
          ${order.message ? `<tr><td style="padding: 8px 0; color: #b8960c; font-weight: bold;">Hilsen:</td><td style="padding: 8px 0;">${order.message}</td></tr>` : ""}
  ...
  await resend.emails.send({
    from: "T&D Bakeri <bestilling@tdbakeri.no>",
    to: order.email,
    subject: "Bestillingen din er mottatt - T&D Bakeri",
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #1a1a1a; color: #f0e6d0; border-radius: 12px; overflow: hidden;">
        <div style="background: linear-gradient(135deg, #b8960c, #8b6914); padding: 32px; text-align: center;">
          <h1 style="margin: 0; color: #fff;">Takk for bestillingen!</h1>
        </div>
        <div style="padding: 32px;">
          <p>Hei ${order.name},</p>
          <p style="margin-top: 16px;">Vi har mottatt bestillingen din og vi gleder oss til a bake kaken din!</p>
          ...
          <p style="margin-top: 24px;">Med venlig hilsen,<br><strong>T&D Bakeri</strong></p>
        </div>
      </div>
    `,
  });
...
export async function sendSmsNotification(order: OrderData): Promise<void> {
  ...
  const body = `NY KAKEBESTILLING! Fra: ${order.name} (${order.phone}). Henting: ${order.pickupDate} kl. ${order.pickupTime}. Betalt depositum. Sjekk e-post for detaljer.`;
