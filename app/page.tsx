    ...
    <>
          ...
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}>
            <div>
              ...
              <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 36 }}>
                {/* Primary: menu + regular ordering */}
                <Link href="/meny" className="btn btn-gold" style={{ padding: "15px 34px", fontSize: "0.92rem" }}>
                  {tr(lang, "hero_cta1")}
                </Link>
                {/* Secondary: custom cake (parallel path) */}
                <Link href="/bestill-kake" className="btn btn-outline">
                  {tr(lang, "hero_cta2")}
                </Link>
              </div>
              <OpenStatus />
      ...
      <section style={{ padding: "96px 0", background: "var(--black)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 28px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 24, marginBottom: 48 }}>
            {items.map(item => (
              <div key={item._id} className="card" style={{ padding: "28px 24px", display: "flex", flexDirection: "column", gap: 0 }}>
                <div style={{ fontSize: "2.2rem", marginBottom: 14 }}>{item.category?.icon ?? "🍰"}</div>
                <div style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 8 }}>{item.category?.title}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 16, borderTop: "1px solid var(--black-border)" }}>
                  ...
                  <Link href="/meny" className="btn btn-outline" style={{ padding: "7px 18px", fontSize: "0.72rem" }}>{tr(lang, "menu_order_btn")}</Link>
                </div>
              </div>
            ))}
          </div>
          <div style={{ textAlign: "center" }}>
            <Link href="/meny" className="btn btn-gold" style={{ padding: "14px 40px" }}>{tr(lang, "menu_see_all")}</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "linear-gradient(160deg,#0e0c02 0%,#060604 50%,#0e0c02 100%)", borderTop: "1px solid rgba(184,150,12,0.12)", borderBottom: "1px solid rgba(184,150,12,0.12)", padding: "88px 28px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", position: "relative" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 32, alignItems: "center" }} className="cta-split">
            {/* Regular ordering */}
            <div style={{ textAlign: "center", padding: "32px 28px", background: "var(--black-card)", border: "1px solid var(--gold-border)", borderRadius: "var(--radius-xl)" }}>
              <div className="section-label" style={{ marginBottom: 12 }}>{tr(lang, "cta_label")}</div>
              <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(1.4rem,2.5vw,1.9rem)", fontWeight: 600, color: "var(--text)", marginBottom: 12, lineHeight: 1.2 }}>
                {tr(lang, "order_type_eat_in")} &amp; {tr(lang, "order_type_take_away")}
              </h2>
              <p style={{ color: "var(--text-muted)", fontSize: "0.88rem", marginBottom: 24, lineHeight: 1.7 }}>
                {tr(lang, "menu_sub")}
              </p>
              <Link href="/meny" className="btn btn-gold">{tr(lang, "hero_cta1")}</Link>
            </div>

            {/* Divider */}
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              <div style={{ width: 1, height: 40, background: "linear-gradient(to bottom,transparent,var(--gold-dim))" }} />
              <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--text-dim)", letterSpacing: "0.12em" }}>ELLER</span>
              <div style={{ width: 1, height: 40, background: "linear-gradient(to bottom,var(--gold-dim),transparent)" }} />
            </div>

            {/* Custom cake */}
            <div style={{ textAlign: "center", padding: "32px 28px", background: "var(--black-card)", border: "1px solid var(--black-border)", borderRadius: "var(--radius-xl)" }}>
              <div className="section-label" style={{ marginBottom: 12 }}>{tr(lang, "order_label")}</div>
              <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(1.4rem,2.5vw,1.9rem)", fontWeight: 600, color: "var(--text)", marginBottom: 12, lineHeight: 1.2 }}>
                {tr(lang, "meny_cta_h2")}
              </h2>
              <p style={{ color: "var(--text-muted)", fontSize: "0.88rem", marginBottom: 24, lineHeight: 1.7 }}>
              ...
              <Link href="/bestill-kake" className="btn btn-outline">{tr(lang, "hero_cta2")}</Link>
            </div>
          </div>
        </div>
        <style>{`@media(max-width:700px){.cta-split{grid-template-columns:1fr !important;}.cta-split>div:nth-child(2){display:none !important;}}`}</style>
      </section>
    </>
  );
}
